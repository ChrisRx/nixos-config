...
in
{
  options.nvidia = {
    ...
    powerLimit = lib.mkOption {
      type = lib.types.int;
      default = 0;
      description = "limit power usage (watts)";
    };
  };
  config = lib.mkIf cfg.enable {
    ...
    # TODO: make enable option
    systemd.services.nvidia-power-limit = lib.mkIf (cfg.powerLimit != 0) {
      description = "Set the power limit for NVIDIA gpu";
      serviceConfig = {
        Type = "oneshot";
        ExecStart = "${pkgs.bash}/bin/bash -c '/run/current-system/sw/bin/nvidia-smi -pm 1 && /run/current-system/sw/bin/nvidia-smi -pl 250'";
      };
