...
in
{
  options.nvidia = {
    ...
    usePatchedPackage = lib.mkEnableOption "Use NVidia package with 6.19 kernel patch";
  ...
  config = lib.mkIf cfg.enable {
    ...
    hardware.nvidia.package = lib.mkIf cfg.usePatchedPackage (
      let
    ...
    systemd.services.nvidia-power-limit = lib.mkIf (cfg.powerLimit != 0) {
      ...
      serviceConfig = {
        Type = "oneshot";
        ExecStart = "${pkgs.bash}/bin/bash -c '/run/current-system/sw/bin/nvidia-smi -pm 1 && /run/current-system/sw/bin/nvidia-smi -pl ${toString cfg.powerLimit}'";
      };
